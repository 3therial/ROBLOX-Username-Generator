...
async function init() {
	await console.log(
		`${colors.RED}\n${colors.UI2}Made by: ${ // if you want fancy thing saying USERNAME GENERATOR then paste ${title.toString()} after ${colors.RED}	
			colors.CYAN
		}Etherial\n${colors.UI1}Send me a DM on discord if you find a bug: eth#4770`
	);
	await checkConfig();
		...
		await rl.question(
			...
			amount => {
				if (!isNaN(amount)) {
					...
					console.log(
						`${colors.RED}Invalid input (val=${amount}).`
					);
					process.exit();
				}
			}
			...
			if (body.message == 'Username is valid') {
				...
				if (config.useDiscordWebhook.enabled) {
					...
					const embed = new MessageEmbed()
						.setTitle('')
						.addField('new user found:', username, true)
						.setColor('CYAN')
						.setFooter('Made by eth#4770')
						.setTimestamp();
